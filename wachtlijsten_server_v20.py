...
CACHE_SECONDS = 10 * 60
FETCH_TIMEOUT = float(os.environ.get("WACHTLIJSTEN_FETCH_TIMEOUT", "25"))
...
def fetch_url(url: str) -> dict[str, Any]:
    ...
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 WachtlijstenTwente/20",
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/health":
            self._json(200, {"ok": True, "version": 20})
            return
